...
public abstract class ChessMoveAbs {
    ...
    public void moveOperate(MoveNode moveNode) {
        ...
        //改全局变位棋盘
        chessParam.maskBoardCaresses.assignXor(MaskChesses[srcSite]);
        chessParam.maskBoardCaresses.assignOr(MaskChesses[destSite]);
        //修改着子方位棋盘
        chessParam.maskBoardPersonalCaresses[srcPlay].assignXor(MaskChesses[srcSite]);
        chessParam.maskBoardPersonalCaresses[srcPlay].assignXor(MaskChesses[destSite]);
        //修改此角色的位棋盘
        chessParam.maskBoardPersonalRoleCaresses[srcChessRole].assignXor(MaskChesses[srcSite]);
        chessParam.maskBoardPersonalRoleCaresses[srcChessRole].assignXor(MaskChesses[destSite]);
        ...
        if (destChess != NOTHING) {
            ...
            //减少棋子数量
            chessParam.reduceCaressesNumb(destChessRole);
            //修改被吃方的位棋盘
            chessParam.maskBoardPersonalCaresses[destPlay].assignXor(MaskChesses[destSite]);
            //修改被吃方的角色位棋盘
            chessParam.maskBoardPersonalRoleCaresses[destChessRole].assignXor(MaskChesses[destSite]);
        }
    ...
    public void unMoveOperate(MoveNode moveNode) {
        ...
        chessParam.baseScore[destPlay] += evaluateCompute.chessAttachScore(destChessRole, destSite);
        //改全局变位棋盘(将目标位置还原)
        chessParam.maskBoardCaresses.assignXor(MaskChesses[destSite]);
        //修改着子方位棋盘
        chessParam.maskBoardPersonalCaresses[destPlay].assignXor(MaskChesses[srcSite]);
        chessParam.maskBoardPersonalCaresses[destPlay].assignXor(MaskChesses[destSite]);
        //修改着子方角色位棋盘
        chessParam.maskBoardPersonalRoleCaresses[destChessRole].assignXor(MaskChesses[srcSite]);
        chessParam.maskBoardPersonalRoleCaresses[destChessRole].assignXor(MaskChesses[destSite]);
        ...
        if (srcChess != NOTHING) {
            ...
            chessParam.baseScore[srcPlay] += evaluateCompute.chessAttachScore(srcChessRole, srcSite);
            //还原棋子数量
            chessParam.increaseCaressesNumb(srcChessRole);
            //将被吃子一方的棋子在位棋盘上还原
            chessParam.maskBoardPersonalCaresses[srcPlay].assignXor(MaskChesses[srcSite]);
            //将被吃子一方的棋子在角色位棋盘上还原
            chessParam.maskBoardPersonalRoleCaresses[srcChessRole].assignXor(MaskChesses[srcSite]);
    ...
    public boolean checked(int play) {
        ...
        if (!BitBoard.assignAndToNew(chessParam.getBitBoardByPlayRole(opponentPlay, ChessConstant.KNIGHT), bitBoard).isEmpty()) {
            ...
            if (knight1Site != NOTHING && !BitBoard.assignAndToNew(MaskChesses[knight1Site], bitBoard).isEmpty()) {
                BitBoard legBoard = BitBoard.assignAndToNew(KnightLegBitBoards[knight1Site], chessParam.maskBoardCaresses);
                //看马1能攻击到的位置是否有将
            ...
            if (knight2Site != NOTHING && !BitBoard.assignAndToNew(MaskChesses[knight2Site], bitBoard).isEmpty()) {
                BitBoard legBoard = BitBoard.assignAndToNew(KnightLegBitBoards[knight2Site], chessParam.maskBoardCaresses);
                //看马1能攻击到的位置是否有将
    ...
    public BitBoard chessAttackSite(int chessRole, int srcSite, int play) {
        BitBoard bitBoard = null;
        bitBoard = EvaluateCompute.getBitBoard(chessRole, srcSite, play, bitBoard, chessParam);
        return bitBoard;
    }
    ...
    public void chessEatMove(int chessRole, int srcSite, int play) {
        ...
        switch (chessRole) {
            ...
            case BLACKCHARIOT:
                ...
                bitBoard = BitBoard.assignXorToNew(ChariotBitBoardOfAttackRow[srcSite][row], ChariotBitBoardOfAttackCol[srcSite][col]);
                //取出攻击到的对方棋子
                bitBoard.assignAnd(chessParam.maskBoardPersonalCaresses[1 - play]);
                break;
            case REDKNIGHT:
            case BLACKKNIGHT:
                //取出被别马腿的位置
                BitBoard legBoard = BitBoard.assignAndToNew(KnightLegBitBoards[srcSite], chessParam.maskBoardCaresses);
                //取出能攻击到对方棋子位置
                bitBoard = BitBoard.assignAndToNew(KnightBitBoardOfAttackLimit[srcSite][legBoard.checkSumOfKnight()], chessParam.maskBoardPersonalCaresses[1 - play]);
                break;
            case REDGUN:
            case BLACKGUN:
                row = chessParam.boardBitRow[boardRow[srcSite]];
            ...
            case BLACKELEPHANT:
                ...
                legBoard = BitBoard.assignAndToNew(ElephanLegBitBoards[srcSite], chessParam.maskBoardCaresses);
                //取出能攻击到对方棋子位置
                bitBoard = BitBoard.assignAndToNew(ElephanBitBoardOfAttackLimit[srcSite][legBoard.checkSumOfElephant()], chessParam.maskBoardPersonalCaresses[1 - play]);
                break;
            case REDKING:
            case BLACKKING:
                //将吃子着法
                bitBoard = BitBoard.assignAndToNew(KingBitBoard[srcSite], chessParam.maskBoardPersonalCaresses[1 - play]);
                break;
            case REDGUARD:
            case BLACKGUARD:
                bitBoard = BitBoard.assignAndToNew(GuardBitBoard[srcSite], chessParam.maskBoardPersonalCaresses[1 - play]);
                break;
            case REDSOLDIER:
            case BLACKSOLDIER:
                bitBoard = BitBoard.assignAndToNew(SoldiersBitBoard[play][srcSite], chessParam.maskBoardPersonalCaresses[1 - play]);
                break;
            default:
                System.out.println("没有这个棋子:" + srcSite);
        }
    ...
    public void chessNopMove(int chessRole, int srcSite, int play) {
        ...
        switch (chessRole) {
            ...
            case BLACKKNIGHT:
                ...
                //走到的位置有棋子的
                bitBoard = BitBoard.assignAndToNew(attackBoard, chessParam.maskBoardCaresses);
                //拿出走到的位置没有棋子的
            ...
            case BLACKKING:
                //将着到的位置有棋子的
                bitBoard = BitBoard.assignAndToNew(KingBitBoard[srcSite], chessParam.maskBoardCaresses);
                //将着到的位置没有棋子的
            ...
            case BLACKGUARD:
                //走的位置有棋子
                bitBoard = BitBoard.assignAndToNew(GuardBitBoard[srcSite], chessParam.maskBoardCaresses);
                //取出没有棋子的
            ...
            case BLACKSOLDIER:
                //有棋子
                bitBoard = BitBoard.assignAndToNew(SoldiersBitBoard[play][srcSite], chessParam.maskBoardCaresses);
                //取出无棋子
