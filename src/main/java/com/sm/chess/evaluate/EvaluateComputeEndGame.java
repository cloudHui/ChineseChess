...
public class EvaluateComputeEndGame extends EvaluateCompute {
    ...
    public int evaluate(int play) {
        ...
        for (int curplay = 0; curplay < 2; curplay++) {
            int soldierNum = chessParam.getCaressesNumb(curplay, ChessConstant.SOLDIER);
            int gunNum = chessParam.getCaressesNumb(curplay, ChessConstant.GUN);
            int knightNum = chessParam.getCaressesNumb(curplay, ChessConstant.KNIGHT);
            //卒之间相互保护加分
            if (soldierNum >= 2) {
                //卒所能攻击到的位置
            ...
            //对手士的数量
            int opponentGuardNum = chessParam.getCaressesNumb(1 - curplay, ChessConstant.GUARD);
            //炮
            if (gunNum > 0) {
